...
const CONTROL_INPUT_CLASS =
  "w-full rounded border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 focus:border-cyan-400 focus:outline-none";
const CONTROL_BUTTON_CLASS =
  "inline-flex items-center gap-1 rounded border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 transition-colors hover:border-cyan-400/70 hover:text-cyan-300 focus:border-cyan-400 focus:outline-none disabled:opacity-50";

export function TelegramChannelManager({ showDashboardLink = false }: TelegramChannelManagerProps) {
    ...
    <div>
      <header className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-cyan-400">Telegram Channel Manager</h1>
          <p className="text-sm text-slate-400">
            전체 {items.length}개 / 활성 <span className="text-emerald-400">{activeCount}개</span> / Provider {provider}
          </p>
        </div>

        <div className="flex items-center gap-2">
          {showDashboardLink ? (
            <Link href="/" className={CONTROL_BUTTON_CLASS}>
              대시보드 복귀
            </Link>
          ) : null}
          <button type="button" onClick={openCreateModal} className={`${CONTROL_BUTTON_CLASS} text-cyan-400`}>
                  ...
                  <tr key={item.channelId} className="border-b border-slate-800/80 last:border-0">
                    ...
                    <td className="px-3 py-2 text-center">
                      <button
                        ...
                        className={`relative inline-flex h-7 w-14 items-center rounded-full border border-slate-700 bg-slate-900 transition-colors focus:border-cyan-400 focus:outline-none ${
                          item.isActive ? "bg-emerald-500/15" : ""
                        }`}
                      >
                        <span
                          className={`inline-block h-5 w-5 transform rounded-full transition-transform ${
                            item.isActive ? "translate-x-8 bg-emerald-400" : "translate-x-1 bg-slate-300"
                          }`}
                        />
                      </button>
                    </td>
                    <td className="px-3 py-2">
                      <div className="flex items-center justify-center gap-2">
                        <button type="button" onClick={() => openEditModal(item)} className={`${CONTROL_BUTTON_CLASS} px-2 py-1 text-xs`}>
                        ...
                        <button
                          ...
                          className={`${CONTROL_BUTTON_CLASS} px-2 py-1 text-xs text-rose-300 hover:text-rose-200`}
                        >
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-md border border-slate-700 bg-slate-950 p-4 shadow-[0_18px_42px_rgba(2,8,23,0.6)]">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button type="button" onClick={closeModal} className={`${CONTROL_BUTTON_CLASS} p-1`}>
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <input
              ...
              className={`${CONTROL_INPUT_CLASS} mb-3`}
            ...
            <label className="mb-4 flex items-center gap-2 text-sm text-slate-300">
              <input
                ...
                className="h-4 w-4 rounded border border-slate-700 bg-slate-900 text-cyan-400 focus:border-cyan-400 focus:outline-none"
              />
              등록 직후 수집 활성화
            </label>

            <div className="flex items-center justify-end gap-2">
              <button type="button" onClick={closeModal} className={CONTROL_BUTTON_CLASS}>
                취소
              </button>
              <button type="button" onClick={() => void saveChannel()} disabled={saving} className={`${CONTROL_BUTTON_CLASS} text-cyan-400`}>
